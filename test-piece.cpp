...
//   for(int i=0; i<n.getpvBriques()/n.getNbLignes();i++){
          
//     for(int j=0;j<n.getpvBriques()/n.getNbLignes();j++){
//        b.setX(b.getX()+4);
//        plateau.addBrique(b);
//        }
//     b.setX(b.getX()-4*n.getpvBriques()/n.getNbLignes()); 
//     b.setY(b.getY()+2);
// }
...
Commandes : < : Deplacer la palette d'une case a gauche.\n\
  ...
  aide.setCouleurFenetre(WBLUE);
  aide.setCouleurBordure(BBLUE);
  ...
  titre.setCouleurBordure(BBLUE);
  infos.setCouleurBordure(BBLUE);
  infos.setCouleurFenetre(WBLUE);
  
  titre.print(0,0,"Casse-Brique",WBLUE);
